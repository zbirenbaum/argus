...
- Deserializable on request: `sandbox dump-checkpoint --seq N --format json`
...
```
sandbox restore --timestamp <T> --target /data/restore/snapshot-1/
```
Original workspace untouched. Safe for inspection, diffing, forking.

**In-place (undo):**
```
sandbox restore --timestamp <T> --in-place --force
```
...
**Selective:**
```
sandbox restore --timestamp <T> --path /workspace/config.yaml
sandbox restore --timestamp <T> --path /workspace/output/ --in-place
```
