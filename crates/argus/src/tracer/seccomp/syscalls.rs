...
//! Syscall number tables for seccomp filtering.
//!
//! Provides architecture-specific syscall numbers for x86_64 and aarch64.
//! aarch64 uses the generic Linux syscall table which omits many legacy
//! syscalls (open, mkdir, etc.) — only the `*at` variants exist.
...
// ── aarch64 syscall numbers (generic Linux ABI) ─────────────────────
// aarch64 omits legacy syscalls: no open, creat, pipe, dup2, fork,
// vfork, mkdir, rmdir, unlink, chmod, chown, link, symlink, readlink,
// rename. Only the *at / modern variants exist.
...
mod arch {
    ...
    // Process — aarch64 has no fork/vfork syscalls; glibc uses clone.
    ...
    // Blocked (io_uring) — same numbers on all architectures.
    pub const SYS_IO_URING_SETUP: u64 = 425;
    pub const SYS_IO_URING_ENTER: u64 = 426;
    pub const SYS_IO_URING_REGISTER: u64 = 427;
    ...
    pub static TRACED: &[u64] = &[
        ...
        SYS_FCHOWN, SYS_FCHOWNAT,
        SYS_TRUNCATE, SYS_FTRUNCATE,
        SYS_LINKAT, SYS_SYMLINKAT, SYS_READLINKAT,
        // FD management
        SYS_CLOSE, SYS_DUP, SYS_DUP3,
        SYS_PIPE2, SYS_FCNTL,
        // Process
